...
config.tokens = {
  'jwt': {
    // Token name as it is saved in the store.
    sources: {
      html: 'token',
      // The "name" attribute value of the HTML "meta" tag.
      api: '/regenerate_token' // URL to retrieve this token from.

    },
    headerName: 'Authorization',
    // In which header to put this token when requesting API.
    headerModifier: function headerModifier(token) {
      return "JWT ".concat(token);
    },
    // If it's a function, it will be used to format the header.
...
config.useLocalStorage = function () {
  try {
    return !!localStorage;
  } catch (any) {
    return false;
